if RequiredScript == "lib/managers/hudmanagerpd2" then
    ...
    HUDMiniMap.Size = {}
    HUDMiniMap.Size[1] = { 175, 175 }  --Map panel width/height
    HUDMiniMap.Size[2] = { 125, 125 }  --Map panel width/height
    HUDMiniMap.Size[3] = { 225, 225 }  --Map panel width/height
    ...
    HUDMiniMap._INDEX = {
        framing_frame_1 = { --Framing Frame day 1 / Art Gallery
            {
                ...
                y = { -5300, 3700 },
                size = { 1024, 1024 },
                texture = "assets/NepgearsyHUD/minimap_assets/shadow_raid_5",
        ...
        mus = { --The Diamond
            {   --Upper floor
                ...
                y = { -5000, 5000 },
                size = { 2048, 1024 },
                texture = "assets/NepgearsyHUD/minimap_assets/mus_1",
            },
            {   --Lower floor
                x = { -10000, 10000 },
                y = { -5000, 5000 },
                size = { 2048, 1024 },
                texture = "assets/NepgearsyHUD/minimap_assets/mus_2",
            },
            {   --Basement
                x = { -10000, 10000 },
                y = { -5000, 5000 },
                size = { 2048, 1024 },
                texture = "assets/NepgearsyHUD/minimap_assets/mus_3",
            },
            bounding_boxes = {
                {   --Exterior
                    x = { max = -4200 },
        ...
        kenaz = {   --Golden Grin Casino
            {
                ...
                y = { -14450, 8350 },
                size = { 1024, 2048 },
                texture = "assets/NepgearsyHUD/minimap_assets/kenaz_loc_b_df",
            },
            bounding_boxes = {
                {   
                    map_index = 1,
    ...
    function HUDMiniMap:init(parent)
        ...
        self._local_scale = 1
        self._zoom_scale = NepgearsyHUD.Data["NepgearsyHUD_ZoomMinimap_Value"] or 1
        self._minimap_enabled = not NepgearsyHUD.Data["NepgearsyHUD_DisableMinimap_Value"] and true or false

        local hud_assault_corner = managers.hud._hud_assault_corner
        local child_hostage_panel = hud_assault_corner._prev_hud:child("hostages_panel")
        local child_kills_panel = hud_assault_corner._prev_hud:child("kills_panel")
        local child_waves_panel = hud_assault_corner._prev_hud:child("wave_panel")
        ...
        if not self._level_data then
            ...
            if NepgearsyHUD.Data["NepgearsyHUD_ForceMinimap_Value"] then
                self._minimap_enabled = true
            end
        end

        if NepgearsyHUD.Data["NepgearsyHUD_SizeMinimap_Value"] then
            sizes = HUDMiniMap.Size[NepgearsyHUD.Data["NepgearsyHUD_SizeMinimap_Value"]]
        else
            sizes = {175, 175}
        end
        ...
        local current_heist_state = managers.hud._hud_assault_corner.current_heist_state

        self._panel:set_right(self._parent:w())
        self._panel:set_top(40)

        if not HUDMiniMap.SHOW_BLANK_MAP and not self._level_data and not NepgearsyHUD.Data["NepgearsyHUD_ForceMinimap_Value"] or not self._minimap_enabled then
            child_hostage_panel:set_right(self._parent:w() - 2)
            child_kills_panel:set_right(self._parent:w() - 10)

            if hud_assault_corner:is_safehouse_raid() then
                child_waves_panel:set_right(self._parent:w() + 9)
            end
        else
            child_hostage_panel:set_right(self._panel:left() - 2)
            child_kills_panel:set_right(self._panel:left() - 10)

            if hud_assault_corner:is_safehouse_raid() then
                child_waves_panel:set_right(self._panel:left() + 9)
            end
        end
